...
module.exports = async (req, res) => {
  ...
  const num = parseInt(String(q.num), 10) || 1;
  const version = parseInt(String(q.version), 10) || 3;

  const path = `/api/mingli/mbti.php?id=${encodeURIComponent(API_ID)}&key=${encodeURIComponent(API_KEY)}&type=1&version=${version}&num=${num}`;
